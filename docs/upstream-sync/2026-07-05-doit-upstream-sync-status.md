...
- 发布门禁已接入机器可读 JSON：`workbench\upstream-sync\reports\sub2api-release-gate-latest.json`。
- 官方更新观察已接入机器可读 JSON：`workbench\upstream-sync\reports\sub2api-upstream-watch-latest.json`。
- 定制层一致性校验已接入机器可读 JSON：`workbench\upstream-sync\reports\sub2api-customization-check-latest.json`。
- 刷新链路已接入 `-RunCustomizationCheck`，统一入口 `sub2api-dev.ps1 refresh` 默认会在生成 staging 后验证 manifest overlay 与品牌替换一致性。
- 下一步决策已接入机器可读 JSON：`workbench\upstream-sync\reports\sub2api-next-action-latest.json`。
