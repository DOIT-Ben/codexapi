...
- `security.url_allowlist.enabled` - 上流ホスト許可リスト。本番環境では有効のままにしてください
- `security.url_allowlist.allow_insecure_http` - バリデーション無効時に HTTP URL を許可（開発/内部テスト専用）
- `security.url_allowlist.allow_private_hosts` - プライベート/ローカル IP アドレスを許可（信頼済み内部デバッグ専用）
...
本番環境では `security.url_allowlist.enabled=true` を維持し、実際に使う上流リレーのドメインを `upstream_hosts` に追加してください。開発環境や内部テストで一時的に `security.url_allowlist.enabled=false` にした場合でも、システムはデフォルトで最小限の URL バリデーションを行い、**HTTP URL を拒否**して HTTPS のみを許可します。HTTP URL を許可するには、以下を明示的に設定する必要があります:
...
本番環境では `SECURITY_URL_ALLOWLIST_ALLOW_INSECURE_HTTP=true` または `SECURITY_URL_ALLOWLIST_ALLOW_PRIVATE_HOSTS=true` を有効にしないでください。
