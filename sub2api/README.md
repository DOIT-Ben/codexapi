...
- `security.url_allowlist.enabled` for upstream host allowlisting; keep it enabled in production
- `security.url_allowlist.allow_insecure_http` to allow HTTP URLs when validation is disabled; dev/internal testing only
- `security.url_allowlist.allow_private_hosts` to allow private/local IP addresses; trusted internal debugging only
...
For production, keep `security.url_allowlist.enabled=true` and add the upstream relay domains you actually use to `upstream_hosts`. If you temporarily set `security.url_allowlist.enabled=false` for development or internal testing, the system still performs minimal URL validation by default, **rejecting HTTP URLs** and only allowing HTTPS. To allow HTTP URLs, you must explicitly set:
...
Do not enable `SECURITY_URL_ALLOWLIST_ALLOW_INSECURE_HTTP=true` or `SECURITY_URL_ALLOWLIST_ALLOW_PRIVATE_HOSTS=true` in production.
