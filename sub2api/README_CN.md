...
- `security.url_allowlist.enabled` 控制上游域名白名单；生产环境建议保持开启
- `security.url_allowlist.allow_insecure_http` 关闭校验时允许 HTTP URL，仅限开发/内网测试
- `security.url_allowlist.allow_private_hosts` 允许私有/本地 IP 地址，仅限可信内网调试
...
生产环境建议保持 `security.url_allowlist.enabled=true`，并把实际使用的上游中转域名加入 `upstream_hosts`。当你为了开发或内网测试临时设置 `security.url_allowlist.enabled=false` 时，系统仍会执行最小 URL 校验，**拒绝 HTTP URL**，仅允许 HTTPS。要允许 HTTP URL，必须显式设置：
...
生产环境不要启用 `SECURITY_URL_ALLOWLIST_ALLOW_INSECURE_HTTP=true` 或 `SECURITY_URL_ALLOWLIST_ALLOW_PRIVATE_HOSTS=true`。
