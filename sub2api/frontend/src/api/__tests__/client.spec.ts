...
describe('API Client', () => {
  ...
  describe('响应拦截器', () => {
    ...
    it('ops monitoring disabled redirects the operations console to settings', async () => {
      const originalLocation = window.location
      try {
        Object.defineProperty(window, 'location', {
          value: { ...originalLocation, pathname: '/admin/operations', href: '/admin/operations' },
          writable: true,
        })
        ...
        const adapter = vi.fn().mockRejectedValue({
          ...
          config: {
            url: '/admin/ops/dashboard/snapshot-v2',
            headers: {},
          },
        ...
        await expect(apiClient.get('/admin/ops/dashboard/snapshot-v2')).rejects.toEqual(
          expect.objectContaining({
            code: 'OPS_DISABLED',
          })
        )
